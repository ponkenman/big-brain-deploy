...
function EditQuestionCard(props: { game: Game, q: Question, setModalIdToDelete: StateSetter<number | undefined>, setDeleteModalIsVisible: StateSetter<boolean> }) {
  ...
  return (
    <article className="p-4 rounded-lg bg-pink-200 flex flex-row justify-between border border-pink-400 shadow-xs">
      <div>
        <p>
          {props.q.index}
          )
          {' '}
          <span className="font-medium">{props.q.question}</span>
        </p>
        <p>
          Possible answers:
          {props.q.answers.length}
        </p>
        <p>
          Correct answers:
          {props.q.type === QuestionType.JUDGEMENT
            ? (
              (() => {
                return (props.q.answers.find(a => a.correct) as Answer).text
              })()
            )
            : props.q.correctAnswers.length}
        </p>
        <p>
          Duration:
          {props.q.duration}
          {' '}
          {props.q.duration === 1 ? `second` : `seconds`}
        </p>
        <p>
          Points:
          {props.q.points}
      ...
      <div className="flex flex-col gap-2">
        ...
        <IconButton
          ...
          alt="Delete question"
          className="w-5 h-auto hover:opacity-50"
          onClick={() => {
  ...
  return (
    <>
      ...
      <p className="text-gray-600">
        ...
        {' '}
        {durationAgo(new Date(props.game.createdAt), new Date())}
        {' '}
        ago
      </p>
      <p className="text-gray-600">
        ...
        {' '}
        {durationAgo(new Date(props.game.lastUpdatedAt), new Date())}
        {' '}
        ago
      </p>
    </>
...
function GameManager(props: { gameId: string }) {
  ...
  return (game != undefined && (
    <div className="rounded-md bg-gray-200 p-4 my-7">
      <GameStats game={game} />
