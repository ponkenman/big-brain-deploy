...
export default function GameCard(props: { games: Game[], setGamesLength: StateSetter<number>, gameId: number }) {
  ...
  return (
    <article className="rounded-lg bg-white border border-gray-400 h-auto overflow-hidden shadow-lg">
          ...
          <div className="flex flex-row justify-between">
            <div>
              <p className="font-semibold">
                {game.name}
                {' '}
                {currSession && <span className="ml-1 p-1 border border-green-600 text-green-600 text-center rounded-lg text-xs align-[1px]">Active</span>}
              </p>
              <p>
              ...
              <p>
                {game.questions ? calcTotalDuration(game.questions) : '0'}
                {' '}
                seconds
              </p>
            </div>
