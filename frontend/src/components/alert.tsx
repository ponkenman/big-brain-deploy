...
export function AlertMenu(props: { alerts: AlertData[], setAlerts: StateSetter<AlertData[]> }) {
  ...
  return (
    <section className="fixed z-1 right-0 bottom-0 m-5 flex flex-col gap-3 max-h-3/4 overflow-scroll">
...
export function AlertPopup(props: { data: AlertData, alerts: AlertData[], setAlerts: StateSetter<AlertData[]> }) {
  ...
  return (visible && (
    <div role="alert" className={`${props.data.colour ?? `bg-red-200 text-red-950 border border-red-300`} px-3 py-2 rounded-xl flex flex-row justify-between items-center`}>
